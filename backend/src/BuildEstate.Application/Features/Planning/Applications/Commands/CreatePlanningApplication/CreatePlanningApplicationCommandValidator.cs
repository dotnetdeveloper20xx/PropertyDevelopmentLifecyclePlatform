...
/// <summary>
/// Validates CreatePlanningApplicationCommand before handler execution.
/// </summary>
public class CreatePlanningApplicationCommandValidator : AbstractValidator<CreatePlanningApplicationCommand>
{
    public CreatePlanningApplicationCommandValidator()
    {
        ...
        RuleFor(x => x.SiteAddress)
            .MaximumLength(500).When(x => !string.IsNullOrWhiteSpace(x.SiteAddress));

        RuleFor(x => x.PlanningOfficer)
            .MaximumLength(200).When(x => !string.IsNullOrWhiteSpace(x.PlanningOfficer));
    }
}
